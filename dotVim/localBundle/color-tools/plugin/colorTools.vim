...
fun! ColorToolsRgbIndexSeparate(r, g, b)
  if &t_Co == 88
...
" Each r/g/b is 0-255
fun! ColorToolsCTermForRgb(r, g, b)
  ...
  if l:gx == l:gy && l:gy == l:gz
    ...
    if l:dgrey < l:drgb
      ...
      " use the color
      return ColorToolsRgbIndexSeparate(l:x, l:y, l:z)
    endif
  else
...
fun! ColorToolsCTermForHexRgb(rgb)
  let l:r = ('0x' . strpart(a:rgb, 0, 2)) + 0
  let l:g = ('0x' . strpart(a:rgb, 2, 2)) + 0
  let l:b = ('0x' . strpart(a:rgb, 4, 2)) + 0

  return ColorToolsCTermForRgb(l:r, l:g, l:b)
endfun
...
" From 255 based rgb.
fun! ColorToolsGuiForRgb(r, g, b)
  return printf('%02x%02x%02x', a:r, a:g, a:b) 
endfunction

" I think this is not necessary if you use
" synIDattr(synIDtrans(hlID('LineNr')), "#fg")
" instead of "fg"
let s:termColorsToRgb = {
...
fun! ColorToolsHighlight(...)
  ...
  if l:bgArg != ''
    ...
    if l:bgArg =~# '^\(\d\|[a-f]\)\+$'
      let l:bg = " guibg=#" . l:bgArg . " ctermbg=" . ColorToolsCTermForHexRgb(l:bgArg)
    else
      let lower = tolower(l:bgArg)
      if has_key(s:termColorsToRgb, lower)
        let arr = s:termColorsToRgb[lower]
        let l:bg = " guibg=#" . ColorToolsGuiForRgb(arr[0], arr[1], arr[2]) . " ctermbg=" . ColorToolsCTermForRgb(arr[0], arr[1], arr[2])
      else
  ...
  if l:fgArg != ''
    " If color is 'NONE' or "DarkCyan" etc. Check for integers first.
    " Looks like a hexcode
    if l:fgArg =~# '^\(\d\|[a-f]\)\+$'
      let l:fg = " guifg=#" . l:fgArg . " ctermfg=" . ColorToolsCTermForHexRgb(l:fgArg)
    else
      let lower = tolower(l:fgArg)
      if has_key(s:termColorsToRgb, lower)
        let arr = s:termColorsToRgb[lower]
        let l:fg = " guifg=#" . ColorToolsGuiForRgb(arr[0], arr[1], arr[2]) . " ctermfg=" . ColorToolsCTermForRgb(arr[0], arr[1], arr[2])
      else
